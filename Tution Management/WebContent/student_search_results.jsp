    ...
    try
    {
        ...
        if(selected_type.equals("course_and_batch")) {
     		student = s.executeQuery("SELECT roll_no, first_name, last_name, email, phone, DOB, gender, "+
						"address, college_name, batch_name, class, course_name,tenth_perc,parent_id,pass FROM"+
						" login, student NATURAL JOIN batch NATURAL JOIN course WHERE login.uname = student.roll_no and course_id ="+data1+" AND batch_name = '"+data2+"';");           	
        }
        else {
 			student = s.executeQuery("SELECT roll_no, first_name, last_name, email, phone, DOB, gender, "+
 						"address, college_name, batch_name, class, course_name,tenth_perc,parent_id,pass FROM"+
 						" login, student NATURAL JOIN batch NATURAL JOIN course WHERE login.uname = student.roll_no and "+selected_type+" = '"+data1+"';");          
        }
%>
<% if(student.next()) {%>
		...
		<tr>
			...
			<th>X %</th><th>Parent ID</th><th>Password</th>
		</tr>
<% do { %>
		<tr>
			...
			<td style="width:100px"><%=student.getString("address") %></td>
			...
			<td><%=student.getInt("parent_id") %></td>
			<td><%=student.getString("pass") %></td>
		</tr>
<%}while(student.next());%>		
		</table>
